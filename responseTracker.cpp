...
int ResponseTracker::recordInput(const std::string& input) {
...
bool ResponseTracker::isRepeated(const std::string& input) {
...
std::string ResponseTracker::getNudgeMessage() {
    // A small set of varied nudge responses so we don't repeat ourselves either
    static std::vector<std::string> nudges = {
    ...
    std::string msg = nudges[nudgeIndex % nudges.size()];
    nudgeIndex++;
    return msg;
}
